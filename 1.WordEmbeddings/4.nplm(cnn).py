...
n_filters = 10           # the number of filters in Conv1D
filter_size = 2          # the size of the convolution window

# Create a NPLM model
x_input = Input(batch_shape = (None, x_train.shape[1]))
x_embed = Embedding(input_dim=V, output_dim=m, name='emb')(x_input)

x_conv = Conv1D(n_filters, filter_size, padding='same')(x_embed)
x_pool = GlobalMaxPooling1D()(x_conv)
y_output = Dense(V, activation = 'softmax')(x_pool)
